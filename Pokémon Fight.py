...
vida = random.randint(35, 150)
dano = random.randint(3, 40)
...
btn_jogar = tk.Button(frame_principal, text="Jogar", font=("Arial", 25),
                      command=lambda: mudar_para_janela(frame_selecao))
...
#sistema funcional de pegar poke

id_pokemon = coordenadas[pokemon_j1.get()]["id"]

id_pokemon = coordenadas[pokemon_j2.get()]["id"]


with open("id.txt", "r") as ficheiro:
    for linha in ficheiro:
        dados = linha.strip().split(";")

        if int(dados[0]) == id_pokemon:
            print("Encontrado!")
            print(dados)
            break
...
btn_confirmar = tk.Button(frame_selecao, text="Confirmar Seleção", font=("Arial", 14),
                          command=lambda: mudar_para_janela(frame_batalha))
...
btn_Terminar = tk.Button(frame_batalha, text="Finalizar Batalha", font=("Arial", 14),
                         command=lambda: mudar_para_janela(frame_resultados))
...
btn_reiniciar = tk.Button(frame_resultados, text="Voltar ao Menu Principal", font=("Arial", 14),
                          command=lambda: mudar_para_janela(frame_principal))
